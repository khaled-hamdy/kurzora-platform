    ...
    <div className="overflow-x-auto">
      <div className="min-w-[800px]">
        ...
        <div className="grid grid-cols-8 gap-2 mb-2">
          <div className="text-slate-400 text-sm font-medium">{t('signals.asset')}</div>
          <div className="text-slate-400 text-sm font-medium text-center">{t('signals.currentPrice')}</div>
          {timeframes.map(tf => (
            <div key={tf} className={`text-center text-slate-400 text-sm font-medium ${tf === timeFilter ? 'text-emerald-400' : ''}`}>
              {tf}
            </div>
          ))}
          <div className="text-center text-slate-400 text-sm font-medium">{t('signals.finalScore')}</div>
          <div className="text-slate-400 text-sm font-medium">{t('signals.actions')}</div>
        </div>
